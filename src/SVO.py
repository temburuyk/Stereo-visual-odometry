...
if __name__ == "__main__":
	...
	ImT1_disparityA = np.frombuffer(Img_py_buffer, dtype=np.ubyte, \
		count = image_size,offset = 2*image_size).reshape((IMG_HEIGHT,IMG_WIDTH))
	ImT2_disparityA = np.frombuffer(Img_py_buffer, dtype=np.ubyte, \
	...
	for frm in range(startFrame+1, endFrame+1):
		...
		np.copyto(LeftImg,ImT1_L)
		np.copyto(RightImg,ImT1_R)
		...
		np.copyto(LeftImg,ImT2_L)
		np.copyto(RightImg,ImT2_R)
